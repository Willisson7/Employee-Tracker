...
const startOptions = () => {
    return inquirer.prompt(selectPrompt)
        .then((answers) => {
            ...
            if (answers.selection === "Update an Employee Role?") { updateEmployee(); }
...
function viewRoles() {
  ...
  function viewEmployees() {
    db.query('SELECT e.id, e.first_name, e.last_name, r.title, r.salary FROM employee e JOIN roles r ON e.role_id = r.id;', (err, results) => {
        if (err) {
            throw err;
        }
        console.table(results);
...
function deleteEmployee() {
    ...
    inquirer.prompt([
        ...
        .then((answers) => {
            ...
            console.log(`${answers.employee_id} has been removed`);
            next();
        });
    })
};
...
function updateEmployee(){
    ...
    db.query('SELECT * FROM employee', (err, results) => {
        ...
        .then((answers) => {
            let employeeChoice = answers.employee_id;
            ...
            inquirer.prompt([
                {
                    type: 'list',
                    name: 'title',
                    message: 'Please enter the title of the role to update:',
                    choices: choices
                }
            ])
                .then((answers) => {
        let roleChoice = answers.title
                    const query = 'UPDATE employee SET role_id = ? WHERE id = ?';
                    const values = [roleChoice, employeeChoice]
                    db.query(query, values, (error, results, fields) => {
                        if (error) {
                            console.error(error);
                            return;
                        ...
                        console.log(`${results.affectedRows} role(s) updated.`);
                        next();
                    });
